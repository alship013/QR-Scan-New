...
const EventEmitter = (typeof window !== 'undefined' && window.EventEmitter) || class {
  constructor() {
    this.events = {};
  }
  on(event, listener) {
...
const ZXing = (typeof window !== 'undefined' && window.ZXing) || {};
const Visibility = (typeof window !== 'undefined' && window.Visibility) || {
  change: function(callback) {
    document.addEventListener('visibilitychange', () => {
      callback(null, document.hidden ? 'hidden' : 'visible');
    });
  },
  state: function() {
...
const StateMachine = {
  create: function(config) {
    const machine = {
      ...
      can: function(event) {
        const eventConfig = config.events.find(e => e.name === event);
        if (!eventConfig) return false;
        const fromStates = eventConfig.from;
        if (Array.isArray(fromStates)) {
          return fromStates.includes(this.current);
        }
        return fromStates === this.current;
      }
    };

    // Add all event methods
    config.events.forEach(event => {
      machine[event.name] = async function(...args) {
        if (this.can(event.name)) {
          const eventConfig = config.events.find(e => e.name === event.name);
          const toState = typeof eventConfig.to === 'function'
            ? eventConfig.to.apply(this, args)
            : eventConfig.to;

          this.current = toState;

          // Call leave callback
          const leaveCallback = config.callbacks[`onleave${eventConfig.from}`];
          if (leaveCallback) {
            await leaveCallback(...args);
          }

          // Call enter callback
          const enterCallback = config.callbacks[`onenter${toState}`] || config.callbacks[`onentered${toState}`];
          if (enterCallback) {
            await enterCallback(...args);
          }
        }
      };
    });
...
// Make Scanner available globally for browser usage
